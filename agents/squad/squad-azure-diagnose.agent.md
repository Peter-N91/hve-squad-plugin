...
* `skills/squad/references/rules/squad-autonomous.md` defines the read-only posture this role holds and the Mandatory Escalation Triggers it never bypasses. This role never runs a destructive operation, a production change, or a resource mutation, regardless of mode.
* `skills/squad/references/rules/squad-state.md` governs squad state: this role returns findings to the coordinator and never writes state directly. Only the Squad Scribe writes history, on the coordinator's behalf.
