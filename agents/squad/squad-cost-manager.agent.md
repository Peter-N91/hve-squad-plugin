...
Prefer the bundled `azure-pricing` skill for every retail-price lookup. It ships with this package as a pinned external cast dependency (see *External Cast* in `skills/squad/references/profiles-and-packs.md`), so it is present without any consumer action, and it already encodes the Retail Prices filter fields, the region-name normalization, the `isPrimaryMeterRegion` rule, and the Copilot Studio credit formulas. Read it before constructing a query by hand.
